...
export default class UserStorage {
  ...
  async update(id, data) {
    ...
    const sql = `UPDATE users SET ${Object.keys(data)
  ...
  async updateStats(id, data) {
    ...
    const sql = `UPDATE stats SET ${Object.keys(data)
      .map((key, index) => `${key} = $${index + 1}`)
  ...
  async create({ email, username, referrer = null }) {
    if (typeof email !== 'string' || email.length < 6 || email.length > 320 || !this.options.emailRegex.test(email)) {
  ...
  async fetch(options = { count }) {
    let sql = 'SELECT id, username, flags, xp, level, gold, gems FROM users';
    const params = [];
  ...
  async fetchStatsByUserId(id) {
    ...
    const sql = 'SELECT * FROM stats WHERE user_id = $1';
